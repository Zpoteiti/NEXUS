...
///
/// 【平台用户绑定流程】
/// 收到 Telegram 消息时，先用 sender_id（格式："{user.id}|{user.username}"）
/// 调用 db::get_user_by_channel_identity("telegram", sender_id) 查找对应 UserId。
/// 若返回 None（首次接入），则向用户发送绑定引导消息，
/// 要求用户通过 WebUI 登录后在 Settings 页完成渠道绑定，绑定后继续处理消息。
/// 参考 nanobot：nanobot/channels/telegram.py sender_id 构造；
///              nanobot/channels/base.py is_allowed()（L79-87）。
///
/// 【Forum Topic / Thread 级别的 Session 隔离】
/// Telegram 超级群组支持 Forum Topic（thread_id）。
/// 不同 topic 应产生独立的 session_key，避免多话题共享上下文：
///   session_key = "telegram:{chat_id}:topic:{thread_id}"（有 thread_id 时）
///   session_key = "telegram:{chat_id}"（普通群聊/私聊）
/// 参考 nanobot：nanobot/channels/telegram.py _derive_topic_session_key()（L525-530）。
///
/// 【发送侧 429 退避】
/// 调用 Telegram Bot API 发送消息时，若收到 HTTP 429，
