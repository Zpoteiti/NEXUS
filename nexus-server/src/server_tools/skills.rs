...
#[async_trait]
impl ServerTool for ReadSkillFileTool {
    fn name(&self) -> &str { "read_skill_file" }
    ...
    fn schema(&self) -> Value {
        json!({
            ...
            "function": {
                ...
                "description": "Read a specific file from a skill's directory. Use when skill instructions reference additional files.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "name": {
    ...
    async fn execute(
        ...
        if name.is_empty() {
            return Err(NexusError::new(ErrorCode::ToolInvalidParams, "read_skill_file: name is required"));
        }
        if file_path.is_empty() {
            return Err(NexusError::new(ErrorCode::ToolInvalidParams, "read_skill_file: file_path is required"));
        }

        // Validate no directory traversal
        if file_path.contains("..") {
            return Err(NexusError::new(ErrorCode::ToolBlocked, "read_skill_file: file_path must not contain '..' (directory traversal)"));
        }
        ...
        let skill_dir = std::path::Path::new(&skill.skill_path);
        ...
        let canonical_dir = skill_dir.canonicalize()
            .map_err(|e| NexusError::new(ErrorCode::ExecutionFailed, format!("skill directory not found: {}", e)))?;
        let canonical_target = target.canonicalize()
            .map_err(|e| NexusError::new(ErrorCode::ToolNotFound, format!("file not found: {}", e)))?;
        if !canonical_target.starts_with(&canonical_dir) {
            return Err(NexusError::new(ErrorCode::ToolBlocked, "read_skill_file: file_path escapes the skill directory"));
        }

        let content = tokio::fs::read_to_string(&canonical_target)
            .await
            .map_err(|e| NexusError::new(ErrorCode::ExecutionFailed, format!("failed to read '{}': {}", file_path, e)))?;
