...
```
User ↔ Browser ↔ nexus-gateway (Rust gateway) ↔ nexus-server ↔ nexus-client(s)
                                                      ↑
                              Discord/Telegram Channel（直连）
```

**nexus-gateway** 是独立的 Rust WebSocket gateway，不是前端框架。浏览器通过 `/ws/browser` 连接，nexus-server 的 `GatewayChannel` 通过 `/ws/nexus` 连接。
...
- ✅ GatewayChannel：连接 nexus-gateway-server，收消息创建 session，发送回复
...
**当前状态**
- ✅ nexus-gateway 独立 Rust gateway（`/ws/browser` + `/ws/nexus` 双端点）
- ✅ GatewayChannel 连接 gateway，双向消息桥接
- ⬜ 浏览器端聊天 UI（HTML/CSS/JS，或轻量前端框架）
- ⬜ 用户认证（JWT 或 session token，gateway 侧验证）
- ⬜ 消息流实时渲染（工具调用过程、流式回复）

**核心文件**
- `nexus-gateway/src/main.rs`（gateway 服务，已完成）
- `nexus-gateway/src/browser.rs`（Browser WS handler，已完成）
- `nexus-gateway/src/gateway.rs`（Nexus WS handler，已完成）
- 待新增：浏览器静态资源（`nexus-gateway/static/`）

**完成后可验证的现象**
- 启动 `cargo run --package nexus-gateway` 后浏览器可访问聊天页面
