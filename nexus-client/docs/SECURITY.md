...
- Secrets in the host environment (`AWS_SECRET_ACCESS_KEY`, `NEXUS_AUTH_TOKEN`, `DATABASE_URL`, etc.) are never visible to agent-executed commands.
...
| Workspace parent (e.g., `~`) | `--tmpfs` | **Masks home directory** -- hides `~/.ssh/`, `~/.nexus/config`, etc. |
| Workspace (e.g., `~/.nexus/workspace`) | `--dir` + `--bind` | Read-write, mounted on top of the parent tmpfs mask |
...
1. **Shell commands** (`guard_command_policy` in `shell.rs`): if the command string contains `../` or `..\`, it's blocked immediately. Absolute paths are extracted token-by-token and checked against workspace + allowed paths. Exceptions: `/dev/null` and `/tmp/nexus*` are always allowed.
